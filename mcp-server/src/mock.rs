...
impl MockStore {
    ...
    fn push_lines(&self, data: &[u8]) {
        ...
        let mut sb = self
            .scrollback
            .lock()
            .expect("mock scrollback lock not poisoned");
...
impl SessionStore for MockStore {
    fn read(&self, req: ReadRequest) -> Result<ReadResponse, String> {
        match req {
            ...
            ReadRequest::ReadClipboard => Ok(ReadResponse::ClipboardContent(
                self.clipboard
                    .lock()
                    .expect("mock clipboard lock not poisoned")
                    .clone(),
            )),
        }
    }
    ...
    fn write_clipboard(&self, text: &str) -> Result<(), String> {
        *self
            .clipboard
            .lock()
            .expect("mock clipboard lock not poisoned") = text.to_string();
        Ok(())
    }
    ...
    fn raise_notification(&self, title: &str, body: &str) -> Result<(), String> {
        *self
            .notification
            .lock()
            .expect("mock notification lock not poisoned") =
    ...
    fn scroll_terminal(&self, _session_id: u32, lines: i32) -> Result<i32, String> {
        let mut offset = self
            .scroll_offset
            .lock()
            .expect("mock scroll_offset lock not poisoned");
        *offset += lines;
        Ok(*offset)
    }

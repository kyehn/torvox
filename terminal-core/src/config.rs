...
mod tests {
    ...
    fn parse_custom_theme_comments_ignored() {
        ...
        content.push_str("# red = #ff0000\n");
        content.push_str("\n");
        content.push_str("foreground = #445566\n");
        let theme = Theme::parse_custom(&content).unwrap();
        assert_eq!(theme.background, [0x11, 0x22, 0x33]);
        assert_eq!(theme.foreground, [0x44, 0x55, 0x66]);
        // Verify comment line with "# key = value" didn't silently set the color
        assert_eq!(
            theme.ansi[1],
            [0, 0, 0],
            "comment with key=value must be ignored"
        );
    }

...
class MainActivity : ComponentActivity() {
   ...
   * SELinux category and cannot write filesDir, emulator-verified). The service writes
   * files/install-result.txt on completion.
   */
  ...
  /**
   * Wire MCP dialog / pick_file requests from the embedded MCP server to UI. Responses are sent
   * back through NativeBridge.dialogResult.
   */
  private fun wireMcpRequestHandlers() {
    runtime.dialogRequestHandler = { sessionId, requestId, dialogType, title, message, options ->
      runOnUiThread {
        try {
    ...
    runtime.dialogCancelHandler = { sessionId, requestId ->
      runOnUiThread {
    ...
    runtime.pickFileRequestHandler = { sessionId, requestId, startingPath, filter ->
      runOnUiThread {
        try {
  ...
  /**
   * Convert an MCP file pattern to an Android MIME-type filter array. Comma-separated patterns are
   * split and each item mapped: items that are already MIME types (`image/*`) pass through; glob
   * patterns `*.txt`) cannot be mapped reliably to MIME types, so they fall back to the unfiltered
   * picker (`*/*`).
   */
  ...
  private val pickFileLauncher =
      registerForActivityResult(
        ...
        if (key != inFlightPickFileKey) {
          Log.w(
              TAG,
              "pick_file: stale ActivityResult callback ignored (key=$key, in-flight=$inFlightPickFileKey)",
          )
          inFlightPickFileKey = null
  ...
  /**
   * Launch the next queued pick_file request, if any. Called after a picker callback answers the
   * current request, keeping launches serialized.
   */

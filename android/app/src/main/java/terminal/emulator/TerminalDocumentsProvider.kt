...
class TerminalDocumentsProvider : DocumentsProvider() {
  companion object {
    ...
    internal fun parseOpenModeFallback(mode: String): Int {
      if (mode.isEmpty() || mode.length > 4) throw IllegalArgumentException("Unsupported mode '$mode'")
      if (mode.any { it !in "rwatsd" }) throw IllegalArgumentException("Unsupported mode '$mode'")
      val read = mode.contains('r')
      val write = mode.contains('w')
      if (!read && !write) throw IllegalArgumentException("Unsupported mode '$mode'")
      var parsed = 0
      parsed =
          parsed or
              if (read && write) ParcelFileDescriptor.MODE_READ_WRITE
              else if (write) ParcelFileDescriptor.MODE_WRITE_ONLY

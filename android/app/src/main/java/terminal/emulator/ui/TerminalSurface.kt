...
constructor(
  ...
  inner class SelectionHandles {
    ...
    private inner class HandleOverlayLayout(
      ...
      private fun handleAt(x: Int, y: Int): HandleDrag? =
          when {
      ...
      private fun lockedIndex(event: MotionEvent): Int =
          dragPointerLocked?.let { event.findPointerIndex(it) } ?: -1
  ...
  fun handleLongPress(
    ...
    @Suppress("DEPRECATION") performHapticFeedback(android.view.HapticFeedbackConstants.LONG_PRESS)
  ...
  override fun onCreateInputConnection(outAttrs: EditorInfo): InputConnection =
      imeConnection.createInputConnection(outAttrs)
 ...
 */
internal fun nextTapCount(now: Long, lastTapTime: Long, tapCount: Int, windowMs: Long): Int =
    if (now - lastTapTime < windowMs) tapCount + 1 else 1
...
internal fun multiTapAction(tapCount: Int): MultiTapAction =
    when {
 ...
 * drag target mapping.
 */
internal fun pixelToCell(px: Float, cellSize: Float, maxCells: Int): Int =
    (px / cellSize).toInt().coerceIn(0, (maxCells - 1).coerceAtLeast(0))

...
class SecondStageRunner(
  ...
  private suspend fun runOnePostinst(
    ...
    try {
      ...
      val stdoutThread =
          Thread { proc.inputStream.bufferedReader().readText() }
              .apply {
                isDaemon = true
              }
  ...
  private fun detectDpkgVersion(): String? {
    ...
    return try {
      ...
      val stderrThread =
          Thread { proc.errorStream.bufferedReader().readText() }
              .apply {
                isDaemon = true
              }
      stderrThread.start()
  ...
  private fun writeTermuxEnv() {
    ...
    envFile.writeText(
        buildString {
              ...
              if (isNixBootstrap) {
                // No NIX_PATH: the bootstrap ships no literal $store/nixpkgs
                // (sources live under hashed *-source dirs); flakes bypass it.

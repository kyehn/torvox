...
/**
 * Smart selection boundary expansion: given a line of terminal text and a tapped column, expands to
 * a whole word / URL / path run.
 *
 * Pure Kotlin so the boundary rules are unit-testable without the native engine. Used by
 * [NativeQueryPort.expandAndSetSelection].
 *
 * References:
 * - termux-app TextSelectionCursorController.setInitialTextSelectionPosition :93-108): whitespace
 *   stops expansion, non-blank expands to the token.
 * - Haven SelectionToolbar.expandSelectionToWord (:40-90) + expandAcrossUrlWrap :120-214):
 *   non-whitespace token + wrapped-URL continuation across rows. The multi-row URL case IS handled
 *   here: [expandAcrossUrlWrap] walks backward/forward across soft-wrapped rows and verifies the
 *   joined run looks like a full URL before overriding the single-row word.
 * - termlib SelectionManager.adjustSelectionForMode (:288-320): mode switch re-expands the range
 *   (WORD → word boundaries) — mirrored in [TerminalViewModel.adjustSelectionForMode].
...
object SelectionExpander {
  ...
  /**
   * Expand a tap at [col] on [line] to a (startCol, endCol) range, clamped to the line bounds.
   *
   * Rules:
   * - URL scheme (`scheme://`) expands to the whole URL run.
   * - Otherwise the maximal non-whitespace run around the tap.
   * - An enclosing quote/bracket pair is trimmed on the tapped side so `"hello"` selects `hello`
   *   and `(word)` selects `word`.
   */
  fun expandBounds(line: String, col: Int): Pair<Int, Int> {
    ...
    if (urlStart >= 0) {
      ...
      while (trimmedEnd > urlStart) {
        ...
        val shouldTrim =
            ch in TRAILING_URL_PUNCTUATION ||
  ...
  /**
   * Strip trailing whitespace AND NUL padding that the renderer uses for empty cells (CellData
   * 空单元格快路径）。
   */
  private fun String.trimTerminalPadding(): String = trimEnd { it.isWhitespace() || it == '\u0000' }

  /**
   * Whether a character commonly appears inside URLs (Haven's SelectionToolbar.isUrlSafe; termlib
   * marks it internal so Haven duplicates it — we do the same for offline testability).
   */
  private fun Char.isUrlSafe(): Boolean = isLetterOrDigit() || this in "/:@!$&'()*+,;=-._~%?#[]"

  /**
   * Bounds of the full URL a single-row word belongs to, walked outward across wrap-continuation
   * rows. Returns null if the word doesn't extend beyond its row, or if the joined text doesn't
   * look like a URL.
   *
   * Mirrors Haven SelectionToolbar.expandAcrossUrlWrap (:120-214):
   * - backward: while the tracked word is the first non-whitespace run on its row and the previous
   *   row's last non-padding char is URL-safe.
   * - forward: while at the row edge and the next row starts with a URL-safe run; an indented run
   *   is a wrap tail only when it's a single short run (hanging-indent shape), not indented prose.
  ...
  @JvmStatic
  internal fun looksLikeFullUrl(s: String): Boolean =
      terminal.emulator.util.UrlToken.looksLikeFullUrl(s)
  ...
  /**
   * Walk backward into previous rows while the tracked word is the first non-whitespace run on its
  ...
  /**
   * Walk forward into next rows while we're at this row's trimmed edge and the next row contributes
   * a URL-safe run. A column-0 run is a classic soft-wrap; an indented run is a wrap tail only when
   * it's a single short run on an otherwise-blank line (hanging-indent shape), not indented prose
   * or a table cell. Returns the walked (row, col).
   */

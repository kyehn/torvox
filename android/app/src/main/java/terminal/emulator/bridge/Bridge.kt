 ...
 */
internal fun argbToRgbFloats(argb: Int): FloatArray =
    floatArrayOf(
...
class Bridge(private val config: TerminalConfig) : TerminalQueryPort {
  ...
  data class PollResult(
    ...
    fun merge(later: PollResult): PollResult =
        PollResult(
  ...
  private fun parseEvent(json: String): PollResult =
      when (val event = pollEventJson.decodeFromString<PollEvent>(json)) {
  ...
  override fun cursorViewportPacked(): Long =
      runCatching { queryPort.cursorViewportPacked() }.getOrDefault(-1L)
  ...
  override fun selectionText(
      ...
      runCatching { queryPort.selectionText(startRow, startCol, endRow, endCol, rectangle) }
          .getOrNull()

  override fun hyperlinkAt(row: Int, col: Int): String? =
      runCatching { queryPort.hyperlinkAt(row, col) }.getOrNull()

  override fun listFontFamilies(): List<String>? =
      runCatching { queryPort.listFontFamilies() }.getOrNull()

  override fun getDefaultFontName(): String =
      runCatching { queryPort.getDefaultFontName() }.getOrDefault("monospace")

...
class ArgumentTokenizerTest {
    ...
    @Test
    fun `trailing backslash outside quotes kept literally`() {
        assertEquals(listOf("a\\"), ArgumentTokenizer.tokenize("a\\"))
        assertEquals(listOf("\\"), ArgumentTokenizer.tokenize("\\"))
    }

    @Test
    fun `trailing backslash combined with unclosed quote`() {
        // Single-quote: `\` inside single quotes is a plain character.
        assertEquals(listOf("a\\"), ArgumentTokenizer.tokenize("'a\\"))
    }

    @Test
    fun `stringify escapes tab cr backspace formfeed and newline`() {
    ...
    @Test
    fun `stringify output has direct format assertions`() {
    ...
    @Test
    fun `tab acts as separator outside quotes`() {
        assertEquals(listOf("a", "b"), ArgumentTokenizer.tokenize("a\tb"))
        assertEquals(listOf("a", "b", "c"), ArgumentTokenizer.tokenize("a\t\tb c"))
    }

    @Test
    fun `empty quote pair inside token yields no empty argument`() {
    ...
    @Test
    fun `cross-quote characters are literal`() {
        assertEquals(listOf("a\"b"), ArgumentTokenizer.tokenize("'a\"b'"))
        assertEquals(listOf("a'b"), ArgumentTokenizer.tokenize("\"a'b\""))
    }

    @Test
    fun `unicode tokens preserved unchanged`() {

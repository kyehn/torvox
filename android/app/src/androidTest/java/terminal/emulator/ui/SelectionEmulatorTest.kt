...
class SelectionEmulatorTest {
    ...
    private fun captureFrame(): terminal.emulator.PixelFrame {
        val bridge = composeTestRule.getBridge() ?: throw AssertionError("Bridge is null")
        bridge.saveTestFrame(dataDir)
        return terminal.emulator.decodeRgbaToPixels(File(dataDir, "test_frame.rgba"))
    }
    ...
    private fun getSelectedText(): String? {
        ...
        composeTestRule.activityRule.scenario.onActivity { activity ->
            ...
            if (content != null) {
                val surface = terminal.emulator.findTerminalSurface(activity)
                if (surface is terminal.emulator.ui.TerminalSurface) {
    ...
    fun longPressOnWordCharacter_selectsWordWithInvertedColors() {
        ...
        for (c in col until col + 5) {
            val actual = terminal.emulator.extractCell(selFrame, c, line, gridCols, gridRows)
            val tmpl = terminal.emulator.extractCell(templateFrame, c, line, gridCols, gridRows)
            val conf = terminal.emulator.matchConfidence(actual, tmpl)
    ...
    fun selectionColors_matchTheme_notHardcoded() {
        ...
        val gridRows = composeTestRule.getBridge()!!.getGridRows()
        val gridCols = composeTestRule.getBridge()!!.getGridCols()
        val selCell = terminal.emulator.extractCell(selFrame, col, line, gridCols, gridRows)
        val tmplCell = terminal.emulator.extractCell(templateFrame, col, line, gridCols, gridRows)
        val conf = terminal.emulator.matchConfidence(selCell, tmplCell)
    ...
    fun selectionHighlight_visibleOnDarkTheme() {
        ...
        val ratio = terminal.emulator.analyzeNonBlackRatio(selFrame)
    ...
    fun selectionAcrossMultiline_highlightSpansMultipleRows() {
        ...
        Thread.sleep(500)
        bridge.saveTestFrame(dataDir)
        val selFrame = terminal.emulator.decodeRgbaToPixels(File(dataDir, "test_frame.rgba"))
        saveScreenshot("multiline-highlight")
        for (r in line1..min(line1 + 2, gridRows - 1)) {
            val selCell = terminal.emulator.extractCell(selFrame, 0, r, gridCols, gridRows)
            val tmplCell = terminal.emulator.extractCell(templateFrame, 0, r, gridCols, gridRows)
            val conf = terminal.emulator.matchConfidence(selCell, tmplCell)
    ...
    fun selectionHighlight_invertsForegroundBackground() {
        ...
        val gridRows = composeTestRule.getBridge()!!.getGridRows()
        val gridCols = composeTestRule.getBridge()!!.getGridCols()
        val selPixels = terminal.emulator.extractCell(selFrame, col, line, gridCols, gridRows)
        val tplPixels = terminal.emulator.extractCell(templateFrame, col, line, gridCols, gridRows)
        val conf = terminal.emulator.matchConfidence(selPixels, tplPixels)
    ...
    fun selectionOCR_verifiesContent() {
        ...
        val bitmap = terminal.emulator.decodeRgbaToBitmap(frameFile)
        try {
    ...
    fun selectionOnColoredText_preservesColorInversion() {
        ...
        val gridRows = composeTestRule.getBridge()!!.getGridRows()
        val gridCols = composeTestRule.getBridge()!!.getGridCols()
        val tmplPixels = terminal.emulator.extractCell(templateFrame, col, line, gridCols, gridRows)
        val selPixels = terminal.emulator.extractCell(selFrame, col, line, gridCols, gridRows)
        val conf = terminal.emulator.matchConfidence(selPixels, tmplPixels)
    ...
    fun selectionBoundaryExactMatch() {
        ...
        terminal.emulator.extractCell(selFrame, col, line, gridCols, gridRows)
        terminal.emulator.extractCell(selFrame, max(0, col - 1), line, gridCols, gridRows)
    ...
    fun selectionOnPartialLine_rightSideNotSelected() {
        ...
        val unselActual = terminal.emulator.extractCell(selFrame, unselCol, line, gridCols, gridRows)
        val unselTmpl = terminal.emulator.extractCell(templateFrame, unselCol, line, gridCols, gridRows)
        val unselConf = terminal.emulator.matchConfidence(unselActual, unselTmpl)
    ...
    fun selectionHighlight_notShownWhenActiveFalse() {
        ...
        val gridCols = composeTestRule.getBridge()!!.getGridCols()
        for (c in col..col + 6) {
            val actual = terminal.emulator.extractCell(inactiveFrame, c, line, gridCols, gridRows)
            val tmpl = terminal.emulator.extractCell(templateFrame, c, line, gridCols, gridRows)
            val conf = terminal.emulator.matchConfidence(actual, tmpl)
    ...
    fun selectionLineExact_boundaryOnLeftEdge() {
        ...
        val gridRows = composeTestRule.getBridge()!!.getGridRows()
        val gridCols = composeTestRule.getBridge()!!.getGridCols()
        val leftCell = terminal.emulator.extractCell(selFrame, 0, line, gridCols, gridRows)
        val leftTmpl = terminal.emulator.extractCell(templateFrame, 0, line, gridCols, gridRows)
        val conf = terminal.emulator.matchConfidence(leftCell, leftTmpl)
    ...
    fun selectionColorDoesNotBleed_toAdjacentCell() {
        ...
        if (bleedCol < gridCols) {
            val adjActual = terminal.emulator.extractCell(selFrame, bleedCol, line, gridCols, gridRows)
            val adjTmpl = terminal.emulator.extractCell(templateFrame, bleedCol, line, gridCols, gridRows)
            val adjConf = terminal.emulator.matchConfidence(adjActual, adjTmpl)
    ...
    fun fastSelectDeselect_noGpuArtifacts() {
        ...
        val ratio = terminal.emulator.analyzeNonBlackRatio(frame)
    ...
    fun selectionOnAlternateBg_inversionLooksCorrect() {
        ...
        val gridCols = composeTestRule.getBridge()!!.getGridCols()
        val selPixels = terminal.emulator.extractCell(selFrame, col, line, gridCols, gridRows)
        val tplPixels = terminal.emulator.extractCell(templateUrlFrame, col, line, gridCols, gridRows)
        val conf = terminal.emulator.matchConfidence(selPixels, tplPixels)

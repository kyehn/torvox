...
- [ ] 1.2 Kotlin `TerminalRuntime`：`ensureMkshPromptRc` 改写 `context.noBackupFilesDir/.mkshrc`；`buildConfig`/`buildFailsafeConfig` 把该路径放入 `TerminalConfig`；`Bridge.kt` `TerminalConfig` 与 `NativeBridge.kt` `initSession` 新增 `mkshrcPath` 参数。
- [ ] 1.3 `ffi.rs` `initSession` 新增 `mkshrcPath` JString 参数并接入 `ShellEnv`（空串→`None`，沿用 `prefix` 模式）；同步 4 处测试调用点传空串。

...
- [ ] 2.1 删 ModifierBar 第二套选择菜单与 TerminalScreen 死接线、删 `computeMenuPosition`/`MenuPosition` 与 `TerminalScreenMenuTest`、修 SelectionSteps 悬空断言；`testDebugUnitTest`+`spotlessCheck`+`detekt` 通过
...
- [ ] 3.1 native 新增 `SelectWordAt`/`SelectLineAt`/`SelectAll` 查询（派生+安装+回传界限）与 gref→绝对坐标反解；cargo 测试（含全选界限不含尾部空行）通过

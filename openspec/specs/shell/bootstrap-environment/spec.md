...
装载器死结（2026-09-07 设备端证实）：bootstrap 内 login/login-inner 均为静态 ELF（`file` 实测），glibc 动态二进制 INTERP 指向 `/nix/store/...-glibc-2.42-84`。端到端证据：会话以 store bash 经 linker64 发起（`SPAWN_LINKER` + `spawn OK`，检测链正常），子进程速死，`fast-death respawn` 自动降级至 `/system/bin/sh`；直接执行 store 二进制报 `Permission denied`（`execute_no_trans`）。直接执行复核（IME 实测）：store 内动态 `nix` 与静态 `proot --version` 均返回 shell RC=126（`Permission denied`），而文件模式位正常（`rwxr-xr-x`，DAC 已排除）——拒绝来自 MAC 层，与 targetSdk 注释的预期相反，以设备行为为准。nix 侧保留安装能力；shell 与 `nixos-rebuild switch` 在此模拟器上不可行，需 root 设备、旧 API 或进程内装载引擎（已评估拒绝）。

...
- **torvox 对照**：**torvox input/KeyboardMode.kt 已实现同款 Secure 配置**（VISIBLE_PASSWORD | NO_SUGGESTIONS | NO_PERSONALIZED_LEARNING，Bridge.kt:699 round-216 注释）——IME 配置等价。torvox 用户曾要求删除键盘模式选择器 UI，但保留 enum——**确认这是正确折中**（IME 配置逻辑保留，选择 UI 删除）

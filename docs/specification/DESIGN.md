...
- **字体选择**：遵循 Android 系统 `fonts.xml`，下面仅为说明，以`fonts.xml`为准：未手动设置时默认使用系统等宽字体。对于 CJK 环境（读取系统设置判定），检查主字体是否支持 CJK；若不支持则配置 CJK 回退字体，并“仅”检查系统默认字体（如对于简体中文用户系统默认字体通常为 noto sans cjk sc，应该设置为 noto sans cjk sc 而不是noto serif cjk sc/noto serif cjk jp/noto serif cjk/noto sans cjk）是否支持 CJK，若支持则将其作为 CJK 回退字体。字体列表支持从字体文件选择，列表中不展示“系统默认”等含糊选项。
  ...
  - 只有一项主字体选择，绝不支持 粗体/斜体 单独设置，粗体/斜体 跟随主字体

- **实际字体信息框**：展示字体的实际使用情况，包括主字体、CJK 回退情况、字体大小与单元格信息。

- **光标闪烁开关**。（当前禁止实现，如果存在相关代码或文档需要删除）

- **光标闪烁速度**：提供调节条，范围与精度须受限。（当前禁止实现，如果存在相关代码或文档需要删除）

- **光标样式**：方块、竖线、下划线。（当前禁止实现，如果存在相关代码或文档需要删除，只需要默认方块，不闪烁）

- **软件主题**：“日间”“夜间”“跟随系统”三种。

- **终端主题**：作用于终端页面与修饰键栏，默认“Dracula Plus 主题”，“跟随系统”开关表示跟随“软件主题”在日 / 夜两种终端主题间切换。
  - 支持用户自定义主题：自定义主题可以修改（支持预览）和删除（未使用状态下，包括名称也可修改）。（当前禁止实现，如果存在相关代码或文档需要删除）
  - 默认主题在第一次使用时即被应用
  - 主题效果支持预览
...
- **终端启动目录**。（默认 /data/data/com.termux/files/home/ 即可，当前禁止实现，如果存在相关代码或文档需要删除）

...
> 本文档记录在 `docs/reference/`（参考仓库调研文档，已全部吸收并删除，去向索引
> 见本文 §9）调研中被**明确拒绝 / 决定不吸收**的技术与特性，
> 附原始出处，供后续评审对照，避免反复争论或误入已否决方向。
> 维护规则：新增拒绝项时须注明原始出处（原 `docs/reference/` 内的文件+行号/章节）并注明拒绝原因。被吸收的旧条目移动到对应吸收文档并在本文档标 `（已吸收）`。
...
| 31 | 自定义字体文件导入（TerminalFontStore 4 槽位文件选择） | `research-ghostty-android-extra.md` §5-1、§7 | **文件导入/私有存储复制**（TerminalFontStore.java）拒绝 —— Rust 侧渲染不接受 Android Typeface，文件导入需 native 存储管理，成本高收益低。**同族 bold/italic 面查找 + 像素合成**已吸收（见 §8 A8） |
...
## 7b. 暂缓 / 低优先（非拒绝，记录备查，本节编号独立于 §8）

| # | 项 | 出处 | 说明 |
|---|----|------|------|
| S1 | DECCKM 预推（应用启动前向 PTY 发送 `\e[?1h\e=`） | `research-zed-port.md` §9、`research-haven.md` §7 | P2：仅在直连 SSH/mosh 时减少首次方向键错位；torvox 当前无 SSH/mosh 直连，且 kitty 等远端已自行处理，暂缓。非拒绝，未来接流式传输时再评估。**注：方向键本身的双套编码已实现**（`getMode` JNI 查 DEC private mode 1 + `TerminalInputEncoder.arrowSequence` SS3/CSI，2026 轮次修复 research-haven.md:141 P2 缺口） |
...
| S6 | emoji 位图渲染（warp Samsung>Noto 优先级 / NotoColorEmoji CBDT 解析） | `research-warp.md` §9、`research-mid-repos-a.md` §1.6 | 更小：`font/cjk.rs` `find_emoji_fallback_fonts` 已收集 NotoColorEmoji 族（`is_emoji_candidate_family` + 加分）但 swash 无法 outline color 字体，渲染时跳过 → 落到 db scan/.notdef。真位图渲染需 CBDT/CDT 解析或第二渲染路径，收益有限（终端 emoji 场景少），暂缓 |
| S7 | 粘贴预览 8000 字符上限（gnome-console kgx-paste-dialog） | `research-gnome-console-extra.md` §2 | P3：torvox 多行/长内容确认对话框预览为 `take(200)+"…"`（`TerminalScreen.kt`）；kgx 用 8000 上限。200 已足够展示意图，暂缓对齐 |
| S8 | OSC 8 URI → 本地 path 解码（zed `try_osc8_url_to_path`） | `research-zed-port.md` §9.3、`research-zed-port-personal.md` | P2：OSC 8 超链接**打开**已实现（`ffi.rs hyperlinkAt` → `TerminalSurface.kt openLinkAt`）；URI→本地文件系统的 path 解码（`file://` + URL 解码 + 存在性检查）无 UI 承载点（点击文件打开编辑器不在路线图），暂缓 |
| S9 | OSC 133 SemanticSegment 列范围 / promptId（termlib） | `research-termlib.md`、`04-termlib-features-adoption.md` | 更小：`getLastCommandOutput` JNI + MCP `last_command_output` 工具已实现字符串捕获（B/C 段文本+exit_code）；逐段列范围/promptId 结构无消费需求，暂缓 |
...
| D7/D12 | mosh 协议文档化、root 会话 | 不实现 | mosh 与 SSH 族拒绝（#14 精神）；单用户终端定位与越权能力冲突 |
...
| D14 | warp 合成提交防抖 | 已覆盖 | `ComposingDiff.reconcile` 已去重（rejected #33） |
...
| D37 | 主题批量同步、备份/恢复加密导出 | 不实施（P3） | 主题编辑器 + 256 色链路已有；备份与会话持久化拒绝（#23 精神） |
...
| R18 | `memchr` 顺手用（osc_handler/action_parser 分隔符） | 原文激进 #9 列为"顺手用（零新增成本）"，但实际未引入：低频路径（OSC 52/8、DECRQSS 参数切分）std `find`/`split` 已够，避免为微优化引入直依赖 |
| R19 | EVENT_QUEUE → flume unbounded | 原文激进 #7 列为可选（"parking_lot 保底、flume 可选"）；parking_lot 已落地覆盖锁路径，自研 `EventQueue`（`event.rs`，Mutex + VecDeque，上限 1024 且 Exit 永不丢）语义保留，未迁移 |
...
| K10 | Molecule 试点 | 原文激进 #3 建议试点（25 个 stateIn 设置流 → 声明式聚合），但 2.2.0 近一年无新 release、社区小众、命令式事件（20+ 处 `_state.update`）无法接管；未实施，仅记录 |
| K11 | 手写 EXIF 修正（未引入） | 背景图解码已换 Coil 3.5.0（`coil-core`），EXIF 方向由 Coil 自动校正，无需手写 ExifInterface |

## 8. 已吸收（历史条目，保留对照；本节编号用 A 前缀，独立于 §7 纯数字编号）
...
## 9. 文档处置索引（吸收自 `docs/review-status.md` §5.2/§5.3，原文已删除）
...
| 明确拒绝 / 用户裁决不要 | 本文档 §1–§7（编号 1–38）+ §8 A1-A10 + §8b（依赖选型） |
| 暂缓 / 低优先 | 本文档 §7b（S1–S11）+ §7c（D 项归档） |

// Infinite LOD grid shader, single-view port of wgpu-example src/grid.wgsl.
// The reference audit (research-wgpu-example.md §6.1) recommends this as the
// off-screen render-verification background: no textures, pure procedural
// anti-aliased grid with 3 LOD levels, distance fog and X/Z axis coloring.
// Uniform layout must stay in sync with `GridUniforms` in pipeline.rs.
...
fn vertex_main(@builtin(vertex_index) vertex_index: u32) -> VertexOutput {
    ...
    let camera_world_pos = ubo.camera_world_pos.xyz;
